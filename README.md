...
将 PDF、Markdown、Word、TXT、图片放入监控目录，DocFlow 自动解析、分块、向量化，通过 Web 界面流式问答，所有数据不离开本机。

Drop PDFs, Markdown, Word docs, TXT files, and images into a watched folder. DocFlow auto-parses, chunks, and indexes them. Ask questions via a streaming web UI — everything stays on your machine.
...
- **多路径监控**：watchdog 监控多个目录，支持递归扫描，新文件自动入库
...
- **Multi-path watching**: watchdog monitors multiple directories with optional recursion
...
| 全文检索 | SQLite FTS5（BM25） | 进程内，O(log N)，无 pickle |
...
├── requirements.txt
├── docflow.db                   # SQLite（文件状态 + chunk 元数据 + FTS5 索引）
...
│       ├── store.py             # SQLite CRUD + FTS5 全文索引
│       ├── queue.py             # 异步 ingest 队列
│       └── watcher.py           # 多目录文件监控（watchdog）
│
