...
The browser shell is split into a small `frontend/index.html`, reusable markup in `frontend/partials/`, product styles in `frontend/app.css`, and focused JavaScript files in `frontend/js/`. New component slices live in `frontend/src/` and are built with Vite into committed browser assets; Settings and the Notes active-review panel are the first migrated slices. Stream parsing is also built from TypeScript and tested with Vitest.
