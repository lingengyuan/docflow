...
`docflow eval public` runs the small committed public-domain smoke benchmark without source filtering. `docflow eval retrieval` runs the larger internal source-filtered project regression set. `docflow eval performance` runs a local parser/chunker smoke check for one long synthetic note and a synthetic many-note library.
