...
def create_router(handlers) -> APIRouter:
    ...
    router.add_api_route(
        "/api/files/{file_id}/metadata", handlers["update_file_metadata"], methods=["PATCH"]
    )
    router.add_api_route("/api/files/batch/favorite", handlers["batch_favorite"], methods=["POST"])
    router.add_api_route(
        "/api/files/batch/metadata", handlers["batch_update_file_metadata"], methods=["POST"]
    )
    router.add_api_route(
        "/api/files/batch/rebuild", handlers["batch_rebuild_files"], methods=["POST"]
    )
    router.add_api_route("/api/file/{file_id}/preview", handlers["preview_file"], methods=["GET"])
    router.add_api_route(
        "/api/file/{file_id}/preview", handlers["preview_file_head"], methods=["HEAD"]
    )
    router.add_api_route(
        "/api/file/{file_id}/chunks", handlers["list_file_chunks"], methods=["GET"]
    )
